...
def process_gagp_bcf_to_nmers_randomize_mut_assignment(species, chr, bed_file, ref_seq, nmer, nmer_mutations):
...
def write_species_mut_dict_to_file(species, chr, snp_type, nmer, mut_dict, nmer_mutations):
	...
	outfilename = './snp_data/randomized_nmer_mutation_counts_maf_filter_exclude_recurrent/' + species + '_' + chr + '_' + snp_type + '_species_' + str(nmer) + 'mer_counts.txt'
...
def write_indiv_mut_dict_to_file(species, chr, snp_type, nmer, indiv_dict, indivs, nmer_mutations):
	...
	outfilename = './snp_data/randomized_nmer_mutation_counts_maf_filter_exclude_recurrent/' + species + '_' + chr + '_' + snp_type + '_indiv_' + str(nmer) + 'mer_counts.txt'
	with open(outfilename, 'w') as outfile:
		outfile.write('indiv\t')
		outfile.write('\t'.join(sorted(nmer_mutations)))
		outfile.write('\n')
