...
def sum_mutation_counts(path, snp_type, nmer, bootstrap=False, delete_files=False):
	
	nmer_mutations = generate_nmer_mutation_list(nmer)
	if bootstrap:
		sample_files = glob.glob(path + '%s_%s_%s_species_%imer_counts_q*.txt' % ('Homo', 'chr1', snp_type, nmer))
		for n in range(len(sample_files)):
			for s in species:
				...
				indivs = []
				for c in chrs:
					species_chr_filename = path + '%s_%s_%s_species_%imer_counts_q%i.txt' % (s, c, snp_type, nmer, n)
					indiv_chr_filename = path + '%s_%s_%s_indiv_%imer_counts_q%i.txt' % (s, c, snp_type, nmer, n)
				...
				with open(path + '%s_%s_species_%imer_counts_q%i.txt' % (s, snp_type, nmer, n), 'w') as outfile_species:
					...
					outfile_species.write('\n')
				with open(path + '%s_%s_indiv_%imer_counts_q%i.txt' % (s, snp_type, nmer, n), 'w') as outfile_indiv:
...
def argument_parse():
	...
	parser.add_argument("bootstrap", help="Is this analysis boostrapped?", type=str2bool, nargs='?', default=False)
...
if __name__ == '__main__':
	...
	args = argument_parse()
	sum_mutation_counts(args.path, args.snp_type, args.nmer, args.bootstrap, args.delete_files)
